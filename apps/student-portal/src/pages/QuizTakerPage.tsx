...
export function QuizTakerPage(): React.ReactElement {
	...
	useEffect(() => {
		...
		async function load(courseId: string): Promise<void> {
			try {
				const q = await getQuiz(courseId);
				setQuiz(q);
			} catch (e: unknown) {
				setError(String(e));
			} finally {
		...
		return (
			<div className="container" style={{ maxWidth: 720 }}>
				<div className="card" style={{ textAlign: "center", padding: "var(--space-8)" }}>
					<div style={{ fontSize: 64, marginBottom: "var(--space-4)" }}>{result.percentage >= 60 ? "🎉" : "📚"}</div>
					...
					{quiz.questions.map((q, i) => {
						...
						return (
							<div
								key={q.id}
								className="card"
								style={{ borderLeft: `4px solid ${fb?.correct ? "var(--color-success)" : "var(--color-error)"}` }}
							>
								...
								<p
									style={{
										...
										color: fb?.correct ? "var(--color-success)" : "var(--color-error)",
									}}
								>
