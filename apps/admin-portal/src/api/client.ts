...
export async function updateSlide(courseId: string, slideId: string, dto: UpdateSlideDto): Promise<SlideDto> {
...
export async function uploadPptx(courseId: string, file: File): Promise<{ slideCount: number }> {
	await keycloak.updateToken(30).catch(() => keycloak.login());
