...
export function CourseDetailPage(): React.ReactElement {
	...
	function startEditSlide(slide: SlideDto): void {
		setEditingSlideId(slide.id);
		setEditTitle(slide.title ?? "");
		setEditRawText(slide.rawText);
	}

	function cancelEditSlide(): void {
		setEditingSlideId(null);
		setEditTitle("");
		setEditRawText("");
	}

	async function handleSaveSlide(slideId: string): Promise<void> {
		if (!id) return;
		setSavingSlide(true);
		setError(null);
		try {
		...
		<div className="container">
			...
			<div className="card" style={{ marginTop: "var(--space-6)" }}>
				...
				{slides.length === 0 ? (
					...
					<div
						style={{ display: "flex", flexDirection: "column", gap: "var(--space-2)", padding: "var(--space-4) 0 0" }}
					>
						{slides.map(slide => (
							<div
								key={slide.id}
							...
							>
								...
								{editingSlideId === slide.id ? (
									<div
										style={{ flex: 1, minWidth: 200, display: "flex", flexDirection: "column", gap: "var(--space-2)" }}
									>
										<input
