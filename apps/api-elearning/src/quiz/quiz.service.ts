...
import type { QuizDto, QuizQuestionDto, QuizResultDto, SubmitQuizDto } from "@cerios/shared-types";
...
export class QuizService {
	...
	async generateQuiz(courseId: string): Promise<QuizDto> {
		...
		const wordPool = this.buildWordPool(course.slides.map(s => s.rawText).join(" "));
		...
		if (questions.length === 0) {
			...
			for (const [i, slide] of course.slides.entries()) {
				questions.push({
					...
					options: [slide.title ?? "Untitled", "Introduction", "Conclusion", "Appendix"],
	...
	async submitQuiz(courseId: string, userId: string, dto: SubmitQuizDto): Promise<QuizResultDto> {
		const quiz = await this.prisma.quiz.findUnique({
			where: { courseId },
		...
		const status = percentage >= 60 ? ("PASSED" as const) : ("FAILED" as const);
	...
	private extractQuestions(
		...
		for (const sentence of sentences.slice(0, 2)) {
			...
			const candidates = words.filter(w => w.length > 4 && !QuizService.STOP_WORDS.has(w.toLowerCase()));
	...
	private pickDistractors(correctWord: string, wordPool: string[]): string[] {
		...
		const candidates = wordPool.filter(w => w.toLowerCase() !== correctWord.toLowerCase());
		const distractors = this.shuffle(candidates).slice(0, 3);

		for (const fallback of fallbackPool) {
			if (distractors.length >= 3) break;
			const alreadyUsed = distractors.some(d => d.toLowerCase() === fallback.toLowerCase());
