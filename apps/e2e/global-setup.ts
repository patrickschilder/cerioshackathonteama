...
async function fetchToken(clientId: string, creds: Credentials): Promise<string> {
	...
	try {
		const response = await context.post(`${KEYCLOAK_URL}/realms/${REALM}/protocol/openid-connect/token`, {
			form: {
				grant_type: "password",
				client_id: clientId,
...
// flow in every test.
async function saveLoginStorageState(portalUrl: string, creds: Credentials, outFile: string): Promise<void> {
...
export default async function globalSetup(_config: FullConfig): Promise<void> {
	...
	await saveLoginStorageState(ADMIN_PORTAL_URL, INSTRUCTOR, path.join(AUTH_DIR, "instructor-admin-portal.json"));
	await saveLoginStorageState(STUDENT_PORTAL_URL, STUDENT, path.join(AUTH_DIR, "student-student-portal.json"));
}
